...
class HiPTRunner:
    ...
    def make_episode_dict(self):
        return {
            ...
            "features" : torch.zeros((self.config.training.rollout_steps, self.config.training.num_envs, 128)).to(self.device),
    ...
    def rollout(self, agent, partner_agents, episode_dict, hi_ep_dict, lstm_state, fracs):
        ...
        for step in range(self.config.training.rollout_steps):
            ...
            with torch.no_grad():
                ...
                features = agent.get_high_feat(obs_agent, next_done, current_lstm_state)
            ...
            episode_dict["features"][step] = features
            ...
            with torch.no_grad():
                disc_logits = agent.get_disc_logits(features)#(obs_agent, next_done, current_lstm_state)
                disc_reward = compute_intrinsic_reward(disc_logits, current_z, scale=self.config.training.disc_rew_scale)

            agent_reward = torch.tensor(agent_reward).to(self.device).view(-1)
            # print(f"Disc Reward: {disc_reward.mean().item()}")
            episode_dict["rewards"][step] = agent_reward + disc_reward
            #episode_dict["rewards"][step] = disc_reward
            episode_dict["disc_rewards"][step] = disc_reward
...
# def compute_intrinsic_reward(disc_logits, current_skill_id, scale=2.0):
#     """
#     disc_logits: [batch_size, num_skills] logits from the discriminator
#     current_skill_id: [batch_size] tensor of integers (skill IDs currently being used)
#     scale: multiplier for reward scaling
#     """
#     # Log-probabilities over skills
#     log_probs = F.log_softmax(disc_logits, dim=-1)  # [B, num_skills]
#     log_probs = log_probs.clamp(min=-10, max=0)
    

#     probabilities = torch.exp(log_probs)
#     intrinsic_reward = probabilities.gather(1, current_skill_id.unsqueeze(1)).squeeze(1)
#     #intrinsic_reward = (intrinsic_reward - intrinsic_reward.mean()) / (intrinsic_reward.std() + 1e-6)


    
#     # Gather log-prob of the actual skill being executed
#     #intrinsic_reward = log_probs.gather(1, current_skill_id.unsqueeze(1)).squeeze(1)
    
#     # Optional: convert to positive by exponentiating or just keep as log-prob
#     intrinsic_reward = scale * intrinsic_reward  # keep as log prob (can be negative)
...
# def compute_intrinsic_reward(disc_logits, current_skill_id, scale=2.0):
#     """
#     Reward = 0 if discriminator predicts another skill.
#     Reward = probability assigned to current skill if discriminator predicts it.
#     """
#     # Logits → probabilities
#     probs = F.softmax(disc_logits, dim=-1)  # [B, num_skills]
    
#     # Predicted skill (argmax)
#     pred_skill = probs.argmax(dim=1)  # [B]
    
#     # Mask: only reward if predicted skill matches current skill
#     mask = (pred_skill == current_skill_id).float()  # 1 if match, 0 otherwise
    
#     # Reward = probability of current skill * mask
#     intrinsic_reward = probs.gather(1, current_skill_id.unsqueeze(1)).squeeze(1) * mask
    
#     # Apply scale
#     intrinsic_reward = scale * intrinsic_reward
    
#     return intrinsic_reward
# import torch
# import torch.nn.functional as F

def compute_intrinsic_reward(disc_logits, executed_skill, K=4, scale=1.0):
    """
    Compute DIAYN-style intrinsic reward:
        r_int = log q_phi(z | s_{t+1}) - log p(z)

    Args:
        disc_logits: [batch_size, K] logits from the discriminator
        executed_skill: [batch_size] long tensor of executed skill IDs (0..K-1)
        K: total number of skills
        scale: optional scaling factor for reward

    Returns:
        intrinsic_reward: [batch_size] tensor
    """
    # Log-probabilities from discriminator
    log_probs = F.log_softmax(disc_logits, dim=-1)  # [B, K]

    # Gather log-probability of the executed skill
    log_q_z = log_probs.gather(1, executed_skill.unsqueeze(1)).squeeze(1)  # [B]

    # Uniform prior
    log_p_z = -torch.log(torch.tensor(float(K), device=disc_logits.device))  # scalar

    # Intrinsic reward
    r_int = scale * (log_q_z - log_p_z)
    return r_int
