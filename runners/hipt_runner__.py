...
from runners.utils import compute_advantage, compute_advantage_hi, compute_advantage_term
...
class HiPTRunner:
    ...
    def make_episode_dict(self):
        return {
            ...
            "current_zs": torch.zeros((self.config.training.rollout_steps, self.config.training.num_envs), dtype=torch.int64).to(self.device),
            # **Add termination info**
            "termination_logits": torch.zeros((self.config.training.rollout_steps, self.config.training.num_envs)).to(self.device),
    ...
    def generate_episode(self, agent, partner_agents, lstm_state, fracs):
        ...
        episode_dict = compute_advantage_term(episode_dict, self.config)
    ...
    def rollout(self, agent, partner_agents, episode_dict, hi_ep_dict, lstm_state, fracs):
        ...
        for step in range(self.config.training.rollout_steps):
            ...
            with torch.no_grad():
                # Sample z and get value
                z, logprob_z, _, value_z, lstm_state, z_prob = agent.get_z_and_value(obs_agent, next_done, current_lstm_state)

                # Sample termination logits and termination value
                #term_logits, term_value, lstm_state = agent.get_termination_and_value(obs_agent, current_z, next_done, current_lstm_state)

                term_action, term_logprob, term_entropy, term_value, _, term_logits = agent.get_termination_and_value(obs_agent, current_z, next_done, current_lstm_state)


                term_probs = torch.sigmoid(term_logits)
                term_dist = torch.distributions.Bernoulli(probs=term_probs)
                term_action = term_dist.sample().to(torch.int64)  # 1=terminate, 0=continue
            ...
            if step == 0:
                ...
                curr_p_len = torch.ones(self.config.training.num_envs, dtype=torch.int32).to(self.device)
                hi_cum_rew = torch.zeros(self.config.training.num_envs).to(self.device)
                current_z = z.clone()
                ...
                # Handle termination decisions and z switches
                for ind in range(self.config.training.num_envs):
                    if term_action[ind].item() == 1:  # termination signal
                        # Switch z
                        current_z[ind] = z[ind]
                        switch_ind[ind] += 1
                        # Append new data for new z segment
            ...
            episode_dict["termination_logits"][step] = term_logits
            episode_dict["termination_actions"][step] = term_action
            episode_dict["termination_values"][step] = term_value
            ...
            # 1. Update length for ongoing skills
            curr_p_len[term_action == 0] += 1

            # # 2. Accumulate reward
            # hi_cum_rew += hi_rew  # always accumulate
            ...
            for ind in range(self.config.training.num_envs):
                if term_action[ind] == 1 or step == self.config.training.rollout_steps - 1:
                    # Skill ends or episode ends
                    skill_len = max(curr_p_len[ind].item(), 1)  # Avoid division by zero
                    avg_rew = hi_cum_rew[ind] / skill_len

                    if "hi_rewards" not in hi_ep_dict[ind]:
                        hi_ep_dict[ind]["hi_rewards"] = avg_rew.view(-1)
                    else:
                        hi_ep_dict[ind]["hi_rewards"] = torch.cat((hi_ep_dict[ind]["hi_rewards"], avg_rew.view(-1)), -1)

                    # Reset for next skill
                    hi_cum_rew[ind] = hi_rew[ind]
                    curr_p_len[ind] = 1  # New skill just started
                else:
                    # Skill continues
                    hi_cum_rew[ind] += hi_rew[ind]
 
                print(f"size of z values: {hi_ep_dict[ind]['z_values'].shape} ; Size of rewards : {hi_ep_dict[ind]['hi_rewards'].shape}")
