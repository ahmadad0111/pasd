...
from runners.utils import compute_advantage, compute_advantage_hi, compute_advantage_term, compute_termination_advantage, compute_termination_reward
...
class HiPTRunner:
    ...
    def make_episode_dict(self):
        return {
            ...
            "termination_advantages_direct": torch.zeros((self.config.training.rollout_steps, self.config.training.num_envs)).to(self.device),
    ...
    def generate_episode(self, agent, partner_agents,lstm_state, fracs):
        ...
        #episode_dict = compute_advantage_term(episode_dict, hi_ep_dict, self.config)
    ...
    def rollout(self, agent, partner_agents, episode_dict, hi_ep_dict, lstm_state, fracs):
        ...
        term_kl_div_rew = np.zeros(self.config.training.num_envs)
        ...
        for step in range(self.config.training.rollout_steps):
            ...
            # we compute for all z's
            with torch.no_grad():
                term_action_full, term_logprob_full, _, term_value_full, _, term_prob_full = agent.get_termination_and_value(obs_agent, expanded_z, next_done, current_lstm_state, expanded = True)
            ...
            # # this is for terrmination cum_kl_rew
            # term_prob_full = torch.cat([term_prob_full[i].view(self.config.training.num_envs,1,-1) for i in range(self.config.layout.z_dim,)],1)
            # term_marginal = torch.matmul(z_prob,term_prob_full).squeeze()

            # term_kl_div_rew = torch.sum(torch.log(term_prob/(term_marginal + 1e-8)) * term_prob , 1)
            # cum_term_kl_rew += term_kl_div_rew.cpu().numpy()

            # this code snipet is used to generate direct advantage for termination and will be directly used as loss

            

            #################### uncomment 
            z_prob_reshaped = z_prob.squeeze(1).permute(1, 0)
            V_hi = (z_prob_reshaped * value_agent_full).sum(dim=0)  # [num_envs]

            # Termination policy advantage (per environment)
            for ind in range(self.config.training.num_envs):
                # Only compute when termination is allowed
                terminate_now = term_action[ind].item() == 1
                current_value = value_agent[ind]  # Value of current option at state s_t
                expected_next_value = V_hi[ind]   # Value if new option is selected

                # Advantage is the benefit of terminating (switching)
                termination_adv = 100 * (expected_next_value - current_value)
                episode_dict['termination_reward'][step, ind] = termination_adv if terminate_now else -termination_adv  
            ...
            #     # Hardcoded reward logic
            #     if terminate_now:
            #         # Penalize termination
            #         episode_dict['termination_reward'][step, ind] = -3.0
            #     else:
            #         # Slightly reward continuation (or zero)
            #         episode_dict['termination_reward'][step, ind] = 1.0
            ...
            # # for termination advatage
            # q_val = value_agent
            # v_val = torch.logsumexp(value_agent_full, dim=0)
            # episode_dict["termination_advantages"][step] = v_val - q_val 
            # episode_dict["termination_returns"][step] = episode_dict["termination_advantages"][step] + episode_dict["termination_values"][step]
            ...
            # uncomment
            for ind in range(self.config.training.num_envs):
                episode_dict['termination_reward'][step, ind] +=  agent_reward[ind]
            ...
            if step == self.config.training.rollout_steps - 1:
                for ind, term in enumerate(term_action):
                    if term == 1:
                        # Add final normalized reward for this high-level option
                        hi_ep_dict[ind]["hi_rewards"] = torch.cat((hi_ep_dict[ind]["hi_rewards"], (hi_cum_rew[ind] / hi_option_step_counter[ind].float()).view(-1)), -1)
                        hi_ep_dict[ind]["hi_rewards"] = torch.cat((hi_ep_dict[ind]["hi_rewards"], hi_rew[ind].view(-1)), -1)
                        hi_option_step_counter[ind] = 1
                    else:
                ...
                for ind, term in enumerate(term_action):
                    if term == 1:
                        if "hi_rewards" not in hi_ep_dict[ind].keys():
                            ...
                            hi_ep_dict[ind]["hi_rewards"] = torch.cat((hi_ep_dict[ind]["hi_rewards"], (hi_cum_rew[ind] / hi_option_step_counter[ind].float()).view(-1)), -1)
                        # Reset for new high-level option
                        hi_cum_rew[ind] = hi_rew[ind]
                        hi_option_step_counter[ind] = 1
                    else:
                        hi_cum_rew[ind] += hi_rew[ind]
            ...
            #         if term == 1:
            #             # Add final normalized reward for this high-level option
            #             hi_ep_dict[ind]["hi_rewards"] = torch.cat(
            #                 (hi_ep_dict[ind]["hi_rewards"], (hi_cum_rew[ind] / hi_option_step_counter[ind].float()).view(-1)), -1
            #             )
            #             hi_ep_dict[ind]["hi_rewards"] = torch.cat(
            #                 (hi_ep_dict[ind]["hi_rewards"], hi_rew[ind].view(-1)), -1
            #             )
            #         else:
            #             hi_cum_rew[ind] += hi_rew[ind]
            #             hi_ep_dict[ind]["hi_rewards"] = torch.cat(
            ...
            #         if term == 1:
            #             if "hi_rewards" not in hi_ep_dict[ind].keys():
            #                 hi_ep_dict[ind]["hi_rewards"] = (hi_cum_rew[ind] / hi_option_step_counter[ind].float()).view(-1)
            #             else:
            #                 hi_ep_dict[ind]["hi_rewards"] = torch.cat(
            #                     (hi_ep_dict[ind]["hi_rewards"], (hi_cum_rew[ind] / hi_option_step_counter[ind].float()).view(-1)), -1
            #                 )
            ...
            #             hi_cum_rew[ind] = hi_rew[ind]
            #             hi_option_step_counter[ind] = 1
            #             #switch_ind[ind] = 0  # Already switched
            #         else:
            #             hi_cum_rew[ind] += hi_rew[ind]
            #             hi_option_step_counter[ind] += 1
