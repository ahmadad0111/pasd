...
from utils.kmeans import OnlineKMeans
...
class HiPTTrainer:
    def __init__(self, config, **kwargs):
        ...
        self.online_kmeans = OnlineKMeans(
            n_clusters=4, #self.config.layout.k_clusters,
            feat_dim=umap_dims, # self.config.model.feature_dim,
            device=self.device
        )
        self.disc_opt = torch.optim.Adam(
            self.agent.discriminator.parameters(),
            lr=config.layout.lr# self.config.training.disc_lr
        )
    ...
    def prepare_batch(self, batch_trajs):
        ...
        batch_trajs["features"] = torch.transpose(batch_trajs["features"], 0, 1).reshape(-1, batch_trajs["features"].shape[-1])
        batch_trajs["disc_rewards"] = torch.transpose(batch_trajs["disc_rewards"], 0, 1).reshape(-1)
    ...
    def train(self):
        ...
        for update in tqdm(range(1, num_updates + 1)):
            ...
            # === Online K-means + discriminator training ===
            features_all = lo_traj["features"].to(self.device)  # shape (N, feat_dim)


            # === 2. Reduce dimensionality for KMeans only ===
            reducer = UMAP(n_components=2, random_state=42)
            features_2d = reducer.fit_transform(features_all.cpu())  # N x 2
            features_2d = torch.tensor(features_2d, device=self.device, dtype=torch.float32)

            # train discriminator with minibatches
            N = features_all.shape[0]
            idxs = torch.randperm(N, device=self.device)
            mb_size = 32# self.config.training.disc_mb_size




            # 1) Update centroids & assign labels for full episode
            with torch.no_grad():
                labels = self.online_kmeans.partial_fit(features_2d)            


            for epoch in range(self.config.training.update_epochs):
                disc_loss_total = 0.0 

                # 2) Train discriminator in minibatches using these labels
                for start in range(0, N, mb_size):
                    end = start + mb_size
                    mb_feats = features_all[start:end].detach()
                    mb_labels = labels[start:end]
                    logits = self.agent.discriminator(mb_feats)
                    loss_disc = F.cross_entropy(logits, mb_labels)
                    self.disc_opt.zero_grad()
                    loss_disc.backward()
                    self.disc_opt.step()

                    disc_loss_total += loss_disc.item() * (end - start)

                avg_disc_loss = disc_loss_total / N
