...
class HiPTTrainer_dyn:
    ...
    def train(self):
        ...
        for update in tqdm(range(1, num_updates + 1)):
            ...
            # print("Term Advantage mean/std:", lo_traj["termination_advantages_direct"].mean(), lo_traj["termination_advantages_direct"].std())
            # print("Term Returns mean/std:", lo_traj["termination_returns"].mean(), lo_traj["termination_returns"].std())
            ...
            for epoch in range(self.config.training.update_epochs):
                ...
                for start in range(0, self.config.training.num_envs, envsperbatch):
                    ...
                    _, newlogprob_term, entropy_term, newvalue_term, _, _ = self.agent.get_termination_and_value(
                        lo_traj["obs"][mb_inds],
                    ...
                    hippo_loss = loss_lo + loss_hi + loss_term
